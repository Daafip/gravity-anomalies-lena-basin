...
def run_initialisation():
    ...
    return _lambda, theta, dh_c1_store, dh_c2_store, dh_s1_store, dh_s2_store, plot_times
...
# # store each month seperately data differences

### shouldn't be the approach as we need the deltas between the wanted periods
# if __name__ == "__main__":
#     #### last full run  514.2109310626984 second - ~10 min
#     #### last run  240. second - ~4 min, n=80
#     print("start")
...
#     if debug:
#         n = 3
#         args = [[_lambda, theta, dh_c1_store[i], dh_c2_store[i], dh_s1_store[i], dh_s2_store[i], True]
#                 for i in range(len(dh_s2_store[:n]))]
#         times = times[:(n + 1)]  # debug
...
#         args = [[_lambda, theta, dh_c1_store[i], dh_c2_store[i], dh_s1_store[i], dh_s2_store[i], True]
#                 for i in range(len(dh_s2_store))]
#         fname1 = "Data\\anomally_waterhead_filtered_multicore_seperate1.nc"
#         fname2 = "Data\\anomally_waterhead_filtered_multicore_seperate2.nc"
#     print("initialisation done")
#     start_time = time.time()
#     print(datetime.datetime.now())
#     with Pool() as pool:
#         store_ewh = pool.map(run_loop, args)
#
#     store_ewh = np.array(store_ewh)
#     store_ewh1 = store_ewh[:,0]
#     store_ewh2 = store_ewh[:,1]
#     finish_time = time.time()
#     print("Program finished in {} seconds - using multiprocessing".format(finish_time - start_time))
#
#     ds1 = xr.DataArray(store_ewh1, dims=("time", "lat", "lon"), coords={"lon": _lambda / np.pi * 180,
#                                                                       "lat": 90 - theta/np.pi*180,
#                                                                       "time": times[1:]}, name="dh(m)")
#     ds2 = xr.DataArray(store_ewh2, dims=("time", "lat", "lon"), coords={"lon": _lambda / np.pi * 180,
#                                                                       "lat": 90 - theta/np.pi*180,
#                                                                       "time": times[1:]}, name="dh(m)")
#
